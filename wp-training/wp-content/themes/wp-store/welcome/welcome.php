<?php
if(!class_exists('WP_Store_Welcome')) :

	class WP_Store_Welcome {
			...
			public function __construct( $plugins, $strings ) {
				...
				$this->tab_sections = array(
					'getting_started' => esc_html__('Getting Started', 'wp-store'),
					'recommended_plugins' => esc_html__('Recommended Plugins', 'wp-store'),
					'demo_import' => esc_html__('Import Demo', 'wp-store'),
					'support' => esc_html__('Support', 'wp-store'),
					'changelog' => esc_html__('Changelog', 'wp-store'),
					'more_wp' => esc_html__('More WordPress Stuff', 'wp-store'),
				);
			...
			public function welcome_admin_notice_display(){
				...
				$wp_store_an_div = '
				<div class="updated wp-store-an notice notice-success is-dismissible">
				%1$s
				<hr/>
				<div class="wp-store-column-wrap">
				<div class="wp-store-column col-img">%2$s</div>
				<div class="wp-store-column col-demos">%3$s</div>
				<div class="wp-store-column col-doc">%4$s</div>
				</div> 
				</div>
				';

				/* translators: 1 - notice title, 2 - notice message */
				$wp_store_an_top = sprintf(
					'<h2>%1$s</h2><p class="about-desc">%2$s</p></hr>',
					esc_html__( 'Congratulations!', 'wp-store' ),
					sprintf(
						/* translators: %s - theme name */
						esc_html__( '%s is now installed and ready to use. We\'ve assembled some links to get you started.', 'wp-store' ),
						$this->theme_name
					)
				);

				$wp_store_an_btn = sprintf(
					/* translators: 1 - url, 2 - button text */
					'<a href="%1$s" class="button button-primary wp-store-btn" >%2$s</a>',
					esc_url( admin_url( 'themes.php?page=welcome-page#demo_import' ) ),
					esc_html__( 'Try one of our ready to use Starter Sites', 'wp-store' )
				);
				$wp_store_an_pbtn = sprintf(
					...
					esc_html__( 'or go to the theme settings', 'wp-store' )
				);
				$wp_store_an_img = sprintf(
					'<picture>
				...
				$wp_store_an_list = sprintf(
					'<div><h3><span class="dashicons dashicons-images-alt2"></span> %1$s</h3><p>%2$s</p></div><div> <p>%3$s</p><p>%4$s</p> </div>',
					__( 'Sites Library', 'wp-store' ),
					// translators: %s - theme name
					sprintf( esc_html__( '%s now comes with a sites library with various designs to pick from. Visit our collection of demos that are constantly being added.', 'wp-store' ), $this->theme_name ),
					$wp_store_an_btn,
					$wp_store_an_pbtn
				);
				$wp_store_an_doc = sprintf(
					'<div><h3><span class="dashicons dashicons-format-aside"></span> %1$s</h3><p>%2$s</p><a class="button wp-store-btn" href="%3$s">%4$s</a></div><div></div>',
					__( 'Documentation', 'wp-store' ),
					// translators: %s - theme name
					sprintf( esc_html__( 'Need more details? Please check our full documentation for detailed information on how to use %s.', 'wp-store' ), $this->theme_name ),
					'https://8degreethemes.com/documentation/wp-store',
					esc_html__( 'Read full documentation', 'wp-store' )
				);
				...
				<div class="wrap about-wrap access-wrap">					
					<div class="top-block-wrap">					
						<div class="about-text">
							<h1><?php /* translators: %1$s: theme name, %2$s: theme version */ printf( esc_html__( 'Welcome to %1$s - Version %2$s', 'wp-store' ), esc_html($this->theme_name), esc_html($this->theme_version) ); ?></h1>
						...
						<div class="welcome-section-wrapper is_loading">
							...
							<div class="notice-sidebar is_loading">
								<div class= "notice-sidebar-item">
									<h4><?php echo esc_html('Join in our social networks!','wp-store') ?></h4>
									<p><?php echo esc_html__('Get connected, share your opinions and more via our social community:', 'wp-store'); ?></p>
									<p><a href="<?php echo esc_url('https://www.facebook.com/8DegreeThemes');?>"><?php echo esc_html('Join our Facebook Group','wp-store') ?></a> <?php echo esc_html('- to receive updates, offers and more.','wp-store') ?></p>
									<p><a href="<?php echo esc_url('https://www.youtube.com/8degreethemes');?>"><?php echo esc_html('Subscribe our YouTube Channel','wp-store') ?></a> <?php echo esc_html('- for tutorials, videos and more.','wp-store') ?></p>
									<p><a href="<?php echo esc_url('https://www.twitter.com/8degreethemes');?>"><?php echo esc_html('Follow us on Twitter','wp-store') ?></a> <?php echo esc_html('- to stay updated.','wp-store') ?></p>
								</div>
								<div class= "notice-sidebar-item">
									<h4><?php echo esc_html('Leave us a review','wp-store') ?></h4>
									<p><?php echo esc_html__('Are you are enjoying WP Corporate? We would love to hear your feedback.', 'wp-store'); ?></p>
									<a href="<?php echo esc_url('https://wordpress.org/support/theme/wp-store/');?>"><?php echo esc_html('Submit a review','wp-store') ?></a>
			...
			public function plugin_activation_callback() {
				...
				if(file_exists($plugin_file)) {
					...
					echo esc_html__('Plugin Does not Exists' , 'wp-store');
				}
