<?php
	...
	$user_id = $_SESSION['id'];
	$sql = 'select `name`,`email`,`address` from `nn_user` where `id` ='.$user_id;
	$rs = mysqli_query($link,$sql);
	$r = mysqli_fetch_assoc($rs);
	
	//Lay cac don hang da dat
...
<div class="account">
    ...
    <div class="account_title">
        <h6 class="bold">Hello, <?php echo $r['name'] ?></h6>
        <p>
    ...
    <div class="acount_info">
        ...
        <div class="big_sec left">
            <div class="big_small_sec left">
                ...
                <a href="?mod=update"><?php echo $r['email'] ?></a><br >
    ...
    <div class="account_table">
		...
		?>
            <ul class="contable">
                ...
                <li class="status last"><?=$TrangThai[$r['status']]?></li>
                <li class="action last">
                <a href="?mod=order&id=<?=$r['id']?>" class="first">View</a>
                <?php
					if($r['status']==0)
					{
				?>
                		<a onClick="return confirm('Bạn có chắc chắn muốn hủy đơn hàng này không?')" href="?mod=order_cancel&id=<?=$r['id']?>">Cancel</a></li>
                <?php
					}
				?>
            </ul>
